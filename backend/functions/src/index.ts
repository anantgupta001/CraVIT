  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      functions.logger.info(
        `FCM sent to vendor_${vendorId} for order ${orderId}`,
      );
    } catch (error) {
      functions.logger.error(
        `Failed to send FCM for new order ${orderId}:`, error,
      );
    }
  });

// 1b) onOrderStatusUpdateNotifyStudent (Firestore trigger)
export const onOrderStatusUpdateNotifyStudent = functions.firestore
  .document("orders/{orderId}")
    ...
    if (after.status === "ready") {
      ...
      if (fcmToken) {
        try {
          ...
          functions.logger.error(
            `Failed to send FCM to student for order ${after.id}:`, error,
          );
        }
      } else {
...
export const acceptOrder = functions.https.onCall(async (data, context) => {
  ...
  if (!context.auth || context.auth.token.role !== "seller" || context.auth.token.vendorId === undefined) {
    throw new functions.https.HttpsError(
      "unauthenticated",
      "User is not authenticated or not a seller with a vendor ID.",
    );
  }
  ...
  return db.runTransaction(async (transaction) => {
    ...
    if (order?.vendorId !== vendorIdFromToken) {
      throw new functions.https.HttpsError(
        "permission-denied",
        "Access denied. Seller does not own this order.",
      );
    }
    ...
    if (order.status !== "placed" && order.status !== "accepted") {
      throw new functions.https.HttpsError(
        "failed-precondition",
        "Order is not in 'placed' or 'accepted' status.",
      );
    }
...
export const completeOrder = functions.https.onCall(async (data, context) => {
  ...
  if (!orderId || !pickupCode) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "Order ID and pickup code are required.",
    );
  }
...
export const setSellerClaims = functions.https.onCall(async (data, context) => {
  // Auth check - only allowed for specific admin UIDs
  if (!context.auth || !ADMIN_UDS.includes(context.auth.uid)) {
    throw new functions.https.HttpsError(
      "permission-denied",
      "Only authorized administrators can set custom claims.",
    );
  }
  ...
  if (!uid || !vendorId) {
    throw new functions.https.HttpsError(
      "invalid-argument",
      "UID and vendorId are required to set seller claims.",
    );
  }
